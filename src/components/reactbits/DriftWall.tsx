...
export default function DriftWall({
  ...
  const columnMeta = useMemo(() => {
    ...
    return columnItems.map(col => {
      ...
      const copies = useCSSAnimation ? 2 : Math.max(3, Math.ceil((containerHeight * 2.2) / copyHeight) + 2);
      return { copyHeight, copies };
    });
  }, [columnItems, tileHeight, gap, containerHeight, useCSSAnimation]);
  ...
  }, [columnMeta, columnItems, useCSSAnimation]);
  ...
  }, [baseVelocities, columnMeta, pauseOnHover, parallax, reduced, applyPlaneTransform, useCSSAnimation]);
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      if (useCSSAnimation || parallax === 0) return;
    ...
    [parallax, reduced, useCSSAnimation]
  );
    ...
    >
      <div ref={planeRef} className="drift-wall__plane">
        {columnItems.map((col, c) => {
          ...
          const isUp = c % 2 === 0;
          const trackClass = useCSSAnimation
            ? `drift-wall__track ${isUp ? 'drift-wall__track--css-up' : 'drift-wall__track--css-down'}`
            : 'drift-wall__track';
