...
class InfiniteGridMenu {
  ...
  run(time = 0) {
    ...
    this.#animId = requestAnimationFrame(t => this.run(t));
  }
  ...
  #init(onInit?: ((instance: InfiniteGridMenu) => void) | null) {
    ...
    if (!glCtx) {
      console.warn('WebGL 2 context unavailable');
      return;
    }
    this.gl = glCtx;
    const gl = this.gl;
...
const DEFAULT_MENU_ITEMS: MenuItem[] = [
  {
    ...
    link: '#rooms',
    title: 'NX Residence',
    description: 'Designer boutique luxury rooms'
  }
];

export default function InfiniteMenu({ items = DEFAULT_MENU_ITEMS, scale = 1.0, onItemClick }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(
        canvas,
        items.length ? items : DEFAULT_MENU_ITEMS,
        handleActiveItem,
        setIsMoving,
        sk => sk.run(),
        scale
  ...
  const handleButtonClick = () => {
    ...
    if (activeItem.link.startsWith('http')) {
      ...
      const element = document.querySelector(activeItem.link);
      if (element) {
